...
#season	= 'JJA'
seasons = ['JJA']#['JJA']#['DJF','MAM','JJA','SON']
...
for season in seasons:
  ...
  #       print 'lead time week '+str(l+1)+' '+curr_mid
    ...
    all_ukmo_ens[yc-1,:,:,:,:,:] = np.swapaxes(np.array(nc_fid.variables['week_t2m'][:]),0,1)
    nc_fid.close()
    ...
    all_ncep_ens[yc-1,:,:,:,:,:] = np.swapaxes(np.array(nc_fid.variables['week_t2m'][:]),0,1)
    nc_fid.close()
    ...
    all_ecmf_ens[yc-1,:,:,:,:,:] = np.swapaxes(np.array(nc_fid.variables['week_t2m'][:]),0,1)			# (11, 5, 20, 3, 11, 40, 47)
    nc_fid.close()
    ...
    all_gpcp[yc-1,:,:,:,:] = np.swapaxes(np.array(nc_fid.variables['week_t2m'][:]),0,1)
    nc_fid.close()
  ...
  # bias correct forecasts for each week
  nsamp = np.zeros((3,nleads))
  nsamp_week = np.zeros((3,nleads,20))# count number of samples at each lead time
        ...
        if week_mid_dates[y,l,w,1] in keep_mon:
          ...
          for e in np.arange(0,ecmf_nmembers):
            ...
            if e < ncep_nmembers:
              ...
              ncep_week_bias[l,w,:,:] = ncep_week_bias[l,w,:,:] + (lag_ncep[y,l,w,e,:,:] - all_gpcp[y,l,w,:,:])

            nsamp[2,l] = nsamp[2,l] + 1
            nsamp_week[2,l,w] = nsamp_week[2,l,w] + 1
            ecmf_week_bias[l,w,:,:] = ecmf_week_bias[l,w,:,:] + (lag_ecmf[y,l,w,e,:,:] - all_gpcp[y,l,w,:,:])
  ...
  '''
  fname_p_gpcp = dir_in+'stats/probabilities/'+region+'_ERA-I_t2m_'+p_name+'_'+season+'_'+str(years[0])+'_'+str(years[len(years)-1])+'.npy'
  ...
  #fname_p_gpcp = dir_in+'stats/probabilities/'+region+'_ERA-I_t2m_'+p_name+'_'+season+'_'+str(years[0])+'_'+str(years[len(years)-1])+'.npy'
  fname_p_ukmo = dir_in+'stats/probabilities/'+region+'_UKMO_t2m_BC_'+p_name+'_'+season+'_'+str(years[0])+'_'+str(years[len(years)-1])+'.npy'
  fname_p_ncep = dir_in+'stats/probabilities/'+region+'_NCEP_t2m_BC_'+p_name+'_'+season+'_'+str(years[0])+'_'+str(years[len(years)-1])+'.npy'
  fname_p_ecmf = dir_in+'stats/probabilities/'+region+'_ECMWF_t2m_BC_'+p_name+'_'+season+'_'+str(years[0])+'_'+str(years[len(years)-1])+'.npy'
          ...
          if week_mid_dates[y,l,w,1] in keep_mon:
            ...
            for e in np.arange(0,ecmf_nmembers):
              if e < 7: # 1) do UKMO
                ...
                tmp_ukmo = []
                tmp_ukmo = all_ukmo_ens[:,l,w,e,:,:] - ukmo_week_bias[l,w,:,:]
                t_ukmo = np.delete(tmp_ukmo,y,axis=0)
                ukmo_percentiles = np.nanpercentile(t_ukmo,percentiles,axis=0)# all_gpcp in (11, 5, 20, 40, 47); gpcp_percentiles in (2, 40, 47)
                curr_ukmo = np.copy(tmp_ukmo[y,:,:].squeeze())
              ...
              if e < 4:
                ...
                tmp_ncep = []
                tmp_ncep = lag_ncep[:,l,w,e,:,:] - ncep_week_bias[l,w,:,:]
                t_ncep = np.delete(tmp_ncep,y,axis=0)
                ncep_percentiles = np.nanpercentile(t_ncep,percentiles,axis=0)
                curr_ncep = np.copy(tmp_ncep[y,:,:])
                for nn in np.arange(0,len(percentiles)+1):
                  if nn == 0:
              ...
              tmp_ecmf = []
              tmp_ecmf = lag_ecmf[:,l,w,e,:,:] - ecmf_week_bias[l,w,:,:]
              t_ecmf = np.delete(tmp_ecmf,y,axis=0)
              ecmf_percentiles = np.nanpercentile(t_ecmf,percentiles,axis=0)
              curr_ecmf = np.copy(tmp_ecmf[y,:,:])
              for nn in np.arange(0,len(percentiles)+1):
                if nn == 0:
