...
years	= [1999]#np.arange(2000,2010+1,1) #years to read and save
...
for y in years:
  ...
  for m in np.arange(0,len(month)):
    ...
    nlats	= len(ukmo_lat)
    nlons	= len(ukmo_lon)
    fname_ukmo= dir_out+region+'_UKMO_ensemble_weekly_hindcasts_lead1-5_'+str(y)+'_'+month[m]+'.nc'
    ...
    nlats	= len(ncep_lat)
    nlons	= len(ncep_lon)
    fname_ncep= dir_out+region+'_NCEP_ensemble_weekly_hindcasts_lead1-5_'+str(y)+'_'+month[m]+'.nc'
    ...
    nlats	= len(ncep_lat)
    nlons	= len(ncep_lon)
    fname_gpcp	= dir_out+region+'_GPCP_weekly_'+str(y)+'_'+month[m]+'.nc'
