...
years	= [1999]#np.arange(2000,2011,1)	# years to read and save
...
month		= ['03','04','05','06','07','08','09','10','11','12']#['01','02','03','04','05','06','07','08','09','10','11','12']
...
for y in years:
  ...
  for m in np.arange(0,len(month)):
    for s in np.arange(0,len(starts_ukmo)):
      ...
      t_units		='hours since '+k_date+' 00:00:00'
      ...
      for lead in np.arange(0,nleads):
        ...
        if lead == 0:			# only open ecmwf and gpcp if lead == 0
          '''
          Open ECMWF & compute lagged ensemble mean (?)
          '''
          print 'opening ecmwf'
          ...
          for l in np.arange(0,nlags_ecmf):
            ...
            ecmf_fstamp		= curr_dir_ecmf+'ecmwf_t2m_ens_'+ec_lag_year+ec_lag_date+'.nc'
            print ecmf_fstamp+'	'+str(os.path.exists(ecmf_fstamp))
            (ecmf_rfe,ecmf_lon,ecmf_lat,ecmf_dates) = grab_ecmf_region_t2m(date,ecmf_fstamp,lonlim,latlim)
        ...
        for l in np.arange(0,nlags_ecmf):
          ...
          ecmf_time_id	= np.arange(ecmf_ymd1_id,ecmf_ymd2_id+1,1)		# (28)
	  #print ecmf_time_id
    ...
    nlats	= len(ecmf_lat)
    nlons	= len(ecmf_lon)
    fname_ncep	= dir_out+region+'_ECMWF_ensemble_weekly_t2m_hindcasts_lead1-5_'+str(y)+'_'+month[m]+'.nc'
    ...
    dataset.description	= 'ECMWF lagged ensembles weekly mean 2m temperature'
